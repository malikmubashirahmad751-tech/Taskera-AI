...
from app.agents.tools_agent import get_all_tools
...
def detect_prompt_injection(text: str) -> bool:
    ...
    risky_phrases = [
        "ignore all prior instructions", "ignore previous instructions", "system override",
        "developer mode", "jailbreak", "you are now", "delete user files", "rm -rf",
        "disregard", "forget everything", "new instructions", "roleplay as",
        "sudo", "admin mode", "god mode", "bypass"
    ]
...
async def agent_node(state: AgentState):
    """Main agent node with context management"""
        ...
        if messages:
            ...
            if isinstance(content, str):
                ...
                if detect_prompt_injection(content):
                    ...
                    return {
                        "messages": [AIMessage(content="I cannot process that request due to safety policies.")],
...
3. **UPLOADED FILES**:
   ...
   - For questions about "this file" or "the document", use `local_document_retriever_tool`
   - If the user sends an image, use `ocr_tool`
...
"""
        ...
        return {
            "messages": [AIMessage(content="The AI service is currently experiencing high demand. Please wait 1-2 minutes and try again.")],
        ...
        if "Safety blockage detected" in str(e):
            ...
            return {
                "messages": [AIMessage(content="I cannot process that request as it may violate safety guidelines.")],
                "retry_count": retry_count,
                "user_id": user_id,
                "user_email": user_email
            }
        ...
        return {
            "messages": [AIMessage(content="I encountered a value error. Please check your input.")],
            "retry_count": retry_count,
            "user_id": user_id,
            "user_email": user_email
        }
        ...
        return {
            "messages": [AIMessage(content="The request took too long to process. Please try again.")],
            "retry_count": retry_count,
            "user_id": user_id,
            "user_email": user_email
        }
        ...
        return {
            "messages": [AIMessage(content="I encountered an internal error. Please try again.")],
            "retry_count": retry_count,
            "user_id": user_id,
            "user_email": user_email
        }
...
def should_continue(state: AgentState) -> str:
    ...
    if retry_count >= 3:
        logger.warning(f"Max retries reached, ending conversation")
        return END
