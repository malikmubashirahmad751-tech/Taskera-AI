...
def schedule_research_task_impl(query: str, run_date_iso: str):
    """(IMPL) Schedules a research task for a specific date and time."""
    try:
        ...
        if not run_date:
            raise ValueError(f"Could not parse date expression: '{run_date_iso}'")
        ...
        logger.warning(f"[Scheduler Impl] Invalid date format for research task: {e}")
        return f"Error: {e}"
    except Exception as e:
...
def manage_calendar_events_impl(
    ...
    """(IMPL) Manage calendar events (create, delete, update, list)."""
    try:
        if action == "create":
